...
2. Database Connection Management 
    - `POST /db/connect`: Connect to a new database and store the connection temporarily (in-memory, encrypted at rest if persisted).
    - `GET /db/list`: Returns active DB connections.
    - `DELETE /db/disconnect/{connection_id}`: Disconnect and delete a connection.
    - `GET /db/schema/{connection_id}`: Get schema/tables for a connection.

3. Query Execution
    - `POST /query/execute`: Execute raw query on a specific DB.

4. LLM-Powered Natural Language Query
    - `POST /query/generate`: Generate and run SQL/Mongo query from natural language prompt. (Rate limited per user/IP)

5. Query History
    - `GET /history/{user_id}`: List past natural language → SQL interactions.
...
1. **Security**
    ...
    - Use JWT or OAuth2 for user authentication.
